  ...
  <div class="user-space">
    ...
    <div class="user-info-panel">
      ...
      <div v-if="selfInfo.id && selfInfo.id !== userInfo.id" class="follow-btn">
        <el-button :type="starFlag ? 'danger' : 'primary'" @click="toggleStar" size="medium"
          style="height: 40px; width: 120px; font-size: 20px;" round>
          {{ starFlag ? '取消关注' : '关注TA' }}
        </el-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    fetchUser() {
      this.$axios.get('/user-api/user/getById/' + this.userId).then(res => {
        const { data } = res;
        if (data.code !== 200) { // Token校验异常
          this.$router.push('/');
        } else {
    ...
    fetchStar() {
      ...
      this.$axios.post('/interaction-api/star/query', starQueryDto).then(res => {
        const { data } = res; // 解构
        if (data.code === 200) {
          // 代表没有收藏
    ...
    toggleStar() {
      ...
      this.$axios.post(`/interaction-api/star/starOperation/${this.userInfo.id}`).then(res => {
        const { data } = res; // 解构
        if (data.code === 200) {
